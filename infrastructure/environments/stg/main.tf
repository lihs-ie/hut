...
module "iam" {
  ...
  service_accounts = {
    ...
    "hut-stg-domain-event-logger" = {
      display_name = "HUT STG Domain Event Logger Service Account"
      description  = "Service account for the STG domain event logger"
      roles = [
        "roles/eventarc.eventReceiver",
      ]
    }
...
module "cloudrun_domain_event_logger" {
  ...
  project_id            = var.project_id
  region                = var.region
  service_name          = "stg-domain-event-logger"
  container_image       = var.domain_event_logger_container_image
  service_account_email = module.iam.service_account_emails["hut-stg-domain-event-logger"]
  allow_unauthenticated = false
...
module "eventarc_domain_event_logger" {
  ...
  project_id            = var.project_id
  region                = var.region
  trigger_name          = "stg-domain-event-logger-trigger"
  pubsub_topic_id       = module.pubsub_app_events.topic_id
...
resource "google_cloud_run_v2_service_iam_member" "domain_event_logger_invoker" {
  ...
  role     = "roles/run.invoker"
  member   = "serviceAccount:${module.iam.service_account_emails["hut-stg-domain-event-logger"]}"
}
