...
/**
 * Apply monorepo tsconfig path aliases to the Storybook Vite config.
 */
const applyViteConfig: StorybookConfig["viteFinal"] = async (config) => {
  const configDir = path.dirname(fileURLToPath(import.meta.url));
  const workspaceRoot = path.resolve(configDir, "..");
  const projects = [
    path.resolve(workspaceRoot, "packages/shared/tsconfig.json"),
    path.resolve(workspaceRoot, "packages/admin/tsconfig.json"),
  ];
...
const config: StorybookConfig = {
  ...
  staticDirs: ["../packages/shared/public", "../packages/admin/public"],
  features: {
