...
run = do
    ...
    defaultPage <- right (newCriteria AllArticles 1 Nothing)
    ...
    forM_ [(0, Nothing), (-1, Just 10), (1, Just 0), (1, Just (-1)), (1, Just 101), (maxBound, Just 100)] $ \(current, items) ->
        checkFailure (newCriteria AllArticles current items)
    forM_ [1, 100] $ \size -> do
        request <- right (newCriteria AllArticles 2 (Just size))
        check "size boundary and offset" (pageSize request == size && pageOffset request == size)
    second <- right (newCriteria AllArticles 2 (Just 2))
