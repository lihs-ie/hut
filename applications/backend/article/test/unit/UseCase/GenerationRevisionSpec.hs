...
run = do
    ...
    outdated <- Tx.prepareToPublish (dependencies (pure ())) generated
    ...
    writeIORef store (Store 1 (Proofreaded proof) 0)
    prepared <- Tx.prepareToPublish (dependencies (pure ())) generated >>= right
    checkStore "first result commits article and one event" 2 (Ready prepared.article) 1
    duplicate <- Tx.prepareToPublish (dependencies (pure ())) generated
    ...
    edited <- Tx.prepareToPublish (dependencies (pure ())) revision >>= right
    checkStore "manual edit does not append event" 3 (Ready edited.article) 1
    late <- Tx.prepareToPublish (dependencies (pure ())) generated
