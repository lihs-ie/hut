...
run = do
    ...
    forM_ articles $ \article -> do
        result <-
            View.viewArticleForAdmin
                (View.Dependencies (\requested -> check "requested identity" (requested == value) >> pure (Right (Just article))))
                request
                >>= right
        check "full article in every state" (result.article == article)
        checkEmptyEvents result.events
    missing <- View.viewArticleForAdmin (View.Dependencies (const (pure (Right Nothing)))) request
    expectError "not found" (createAggregateNotFound "Article" (articleIdentifierText value)) missing
    failed <- View.viewArticleForAdmin (View.Dependencies (const (pure (Left failure)))) request
    ...
    forM_ articles $ \article -> do
        result <- View.viewArticleForAdmin (View.Dependencies (const (pure (Right (Just article))))) wrong
