...
import Domain.Event
  ( ArticleCreatedPayload (..),
    ...
    EventPayload
      ( ArticleCreatedPayload',
        ArticleEditedPayload',
        ArticleTerminatePayload',
        ChapterCreatedPayload',
        ChapterEditedPayload',
        ChapterTerminatePayload',
        EngagementRecordedPayload',
        MemoCreatedPayload',
        MemoEditedPayload',
        MemoTerminatePayload',
        PageViewRecordedPayload',
        SearchRecordedPayload',
        SeriesCreatedPayload',
        SeriesEditedPayload',
        SeriesTerminatePayload',
        TagPersistedPayload',
    ...
    MemoCreatedPayload (..),
    MemoEditedPayload (..),
    MemoEntry (MemoEntry, createdAt, text),
    PageViewRecordedPayload (..),
    SearchRecordedPayload (..),
    SeriesChapter (..),
    SeriesCreatedPayload (..),
...
data TagPersistedPayloadRequest = TagPersistedPayloadRequest
  { identifier :: String,
    name :: String
  }
  deriving (Generic)

instance FromJSON TagPersistedPayloadRequest

newtype TagTerminatedPayloadRequest = TagTerminatedPayloadRequest
  { tag :: String
  }
  deriving (Generic)

instance FromJSON TagTerminatedPayloadRequest

data PageViewRecordedPayloadRequest = PageViewRecordedPayloadRequest
  { path :: String,
    count :: Int
  }
  deriving (Generic)

instance FromJSON PageViewRecordedPayloadRequest

data UniqueVisitorRecordedPayloadRequest = UniqueVisitorRecordedPayloadRequest
  { path :: String,
    count :: Int
  }
  deriving (Generic)

instance FromJSON UniqueVisitorRecordedPayloadRequest

data EngagementRecordedPayloadRequest = EngagementRecordedPayloadRequest
  { path :: String,
    duration :: Int
  }
  deriving (Generic)

instance FromJSON EngagementRecordedPayloadRequest

data SearchRecordedPayloadRequest = SearchRecordedPayloadRequest
  { query :: String,
    count :: Int
  }
  deriving (Generic)

instance FromJSON SearchRecordedPayloadRequest
...
toDomain request = case request.eventType of
  ...
  "tag.persisted" -> do
    (wrapper :: TagPersistedPayloadRequest) <- parsePayload request.payload
    Right $
      makeEvent
        request
        TagPersisted
  ...
  "tag.terminated" -> do
    (wrapper :: TagTerminatedPayloadRequest) <- parsePayload request.payload
    Right $
      makeEvent
        request
        TagTerminated
  ...
  "pageView.recorded" -> do
    (wrapper :: PageViewRecordedPayloadRequest) <- parsePayload request.payload
    Right $
      makeEvent
        request
        PageViewRecorded
  ...
  "uniqueVisitor.recorded" -> do
    (wrapper :: UniqueVisitorRecordedPayloadRequest) <- parsePayload request.payload
    Right $
      makeEvent
        request
        UniqueVisitorRecorded
  ...
  "engagement.recorded" -> do
    (wrapper :: EngagementRecordedPayloadRequest) <- parsePayload request.payload
    Right $
      makeEvent
        request
        EngagementRecorded
  ...
  "search.recorded" -> do
    (wrapper :: SearchRecordedPayloadRequest) <- parsePayload request.payload
    Right $
      makeEvent
        request
        SearchRecorded
