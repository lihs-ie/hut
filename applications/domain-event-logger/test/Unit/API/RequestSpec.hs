...
spec = do
  describe "toDomain" $ do
    ...
    context "tag.persisted" $ do
      it "converts to TagPersisted event" $ do
        let payload = object ["identifier" .= ("tag-1" :: String), "name" .= ("haskell" :: String)]
            json = mkEventJSON 1 "tag.persisted" payload
        case parseAndConvert json of
          Left err -> expectationFailure err
          Right event -> do
            event.eventType `shouldBe` TagPersisted
            case event.payload of
              TagPersistedPayload' tag -> do
                tag.identifier `shouldBe` "tag-1"
                tag.name `shouldBe` "haskell"
              _ -> expectationFailure "Expected TagPersistedPayload'"

    context "tag.terminated" $ do
      it "converts to TagTerminated event" $ do
        let payload = object ["tag" .= ("tag-1" :: String)]
            json = mkEventJSON 1 "tag.terminated" payload
        case parseAndConvert json of
          Left err -> expectationFailure err
          Right event -> do
            event.eventType `shouldBe` TagTerminated
            case event.payload of
              TagTerminatedPayload' tag ->
                tag.tag `shouldBe` "tag-1"
              _ -> expectationFailure "Expected TagTerminatedPayload'"

    context "pageView.recorded" $ do
      it "converts to PageViewRecorded event" $ do
        let payload = object ["path" .= ("/articles/1" :: String), "count" .= (42 :: Int)]
            json = mkEventJSON 1 "pageView.recorded" payload
        case parseAndConvert json of
          Left err -> expectationFailure err
          Right event -> do
            event.eventType `shouldBe` PageViewRecorded
            case event.payload of
              PageViewRecordedPayload' pv -> do
                pv.path `shouldBe` "/articles/1"
                pv.count `shouldBe` 42
              _ -> expectationFailure "Expected PageViewRecordedPayload'"

    context "uniqueVisitor.recorded" $ do
      it "converts to UniqueVisitorRecorded event" $ do
        let payload = object ["path" .= ("/articles/1" :: String), "count" .= (10 :: Int)]
            json = mkEventJSON 1 "uniqueVisitor.recorded" payload
        case parseAndConvert json of
          Left err -> expectationFailure err
          Right event -> do
            event.eventType `shouldBe` UniqueVisitorRecorded
            case event.payload of
              UniqueVisitorRecordedPayload' uv -> do
                uv.path `shouldBe` "/articles/1"
                uv.count `shouldBe` 10
              _ -> expectationFailure "Expected UniqueVisitorRecordedPayload'"

    context "engagement.recorded" $ do
      it "converts to EngagementRecorded event" $ do
        let payload = object ["path" .= ("/articles/1" :: String), "duration" .= (120 :: Int)]
            json = mkEventJSON 1 "engagement.recorded" payload
        case parseAndConvert json of
          Left err -> expectationFailure err
          Right event -> do
            event.eventType `shouldBe` EngagementRecorded
            case event.payload of
              EngagementRecordedPayload' eng -> do
                eng.path `shouldBe` "/articles/1"
                eng.duration `shouldBe` 120
              _ -> expectationFailure "Expected EngagementRecordedPayload'"

    context "search.recorded" $ do
      it "converts to SearchRecorded event" $ do
        let payload = object ["query" .= ("haskell" :: String), "count" .= (5 :: Int)]
            json = mkEventJSON 1 "search.recorded" payload
        case parseAndConvert json of
          Left err -> expectationFailure err
          Right event -> do
            event.eventType `shouldBe` SearchRecorded
            case event.payload of
              SearchRecordedPayload' sr -> do
                sr.query `shouldBe` "haskell"
                sr.count `shouldBe` 5
              _ -> expectationFailure "Expected SearchRecordedPayload'"
